...
const InfoScreen = () => {
    ...
    const banner_carouselRef = React.createRef();
    ...
    useEffect(() => {
        ...
        const moveForward = () => {
          if (activeSlide_newsBanner < bannerImages.length - 1) {
            banner_carouselRef.current.snapToNext();
            setActiveSlide_newsBanner(activeSlide_newsBanner + 1);
          } else {
            setIsForward(false); // Change direction to backward
          }
        };
        ...
        const moveBackward = () => {
          if (activeSlide_newsBanner > 0) {
            banner_carouselRef.current.snapToPrev();
            setActiveSlide_newsBanner(activeSlide_newsBanner - 1);
          } else {
            setIsForward(true); // Change direction to forward
          }
        };
        ...
        // Clear interval timer when component unmounts
        return () => clearInterval(timer);
    
    }
      }, [activeSlide_newsBanner, isForward]);

      useEffect(() => {
        // Start the carousel moving forward when the component mounts
        banner_carouselRef.current.snapToNext();
      }, []);
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList style={styles.flatlistContainer}
                    ...
                    <View style={[styles.container, { paddingHorizontal:20}]}>
                        ...
                        <View style={[styles.Carouselcontainer, {paddingTop:60, paddingRight:30, marginRight:10}]}>
                            ...
                            <Carousel
                                ref={(c) => {
                                    if (c) {
                                      banner_carouselRef.current = c;
                                      setCarouselReady(true); // Mark carousel as ready
                                    }
                                  }}
                                data={bannerImages}
                ...
                <Text style={[styles.heading_text,{right:40,paddingBottom:15}]}>Our Partners</Text>
            </View>
